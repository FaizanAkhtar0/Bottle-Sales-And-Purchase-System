...
{
    public partial class CrusherEmployeeManagement : Form
    {
        ...
        public CrusherEmployeeManagement(ref bool param)
        {
            InitializeComponent();
        ...
        private void save_employee_Click(object sender, EventArgs e)
        {
            if (!(txt_employee_name.Text.Equals(""))) {
                ...
                try {
                    ...
                    String conString = "insert into db_plastic_management.dbo.BottleCrusherEmployees(name, cnic, loan, usr_address) values('" + txt_employee_name.Text + "' , '" + txt_employee_cnic.Text + "' , '" + loan + "' , '" + txt_employee_address.Text + "')";
                    SqlCommand cmd = new SqlCommand(conString, con);

                    con.Open();
                    cmd.ExecuteNonQuery();
                    con.Close();
                    MessageBox.Show("Insertion of a new Crusher-employee was sucessful!", "Insertion Sucess", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    refresh();
                    AutoFill();
                } catch (Exception ex) {
        ...
        private void update_employee_Click(object sender, EventArgs e)
        {
            if (!(txt_employee_name.Text.Equals("")) && !(this.ID == -1)) {
                try {
                    String conString = "update db_plastic_management.dbo.BottleCrusherEmployees set name = '" + txt_employee_name.Text + "', cnic = '" + txt_employee_cnic.Text + "', loan = '" + txt_employee_loan.Text + "', usr_address = '" + txt_employee_address.Text + "' where e_id = '" + this.ID + "'";
                    SqlCommand cmd = new SqlCommand(conString, con);
        ...
        private void delete_employee_Click(object sender, EventArgs e)
        {
            if (!(txt_employee_name.Text.Equals(""))) {
                try {
                    String conString = "delete from db_plastic_management.dbo.BottleCrusherEmployees where e_id = '" + this.ID + "'";
                    SqlCommand cmd = new SqlCommand(conString, con);
        ...
        {
            try {
                String conString = "select * from db_plastic_management.dbo.CrusherWork";
                SqlCommand cmd = new SqlCommand(conString, con);
        ...
        {
             try {
                String conString = "select * from db_plastic_management.dbo.BottleCrusherEmployees";
                SqlCommand cmd = new SqlCommand(conString, con);
        ...
        private void employee_save_job_Click(object sender, EventArgs e)
        {
            if (!(job_assignment_employee_id.Text.Equals("")) && !(job_assignment_seller_id.Text.Equals("")) && !(txt_separated_quantity.Text.Equals("") && !(txt_advance_money.Text.Equals("")))) {
                ...
                try {
                    String conString = "select pay, separatedQuantity from db_plastic_management.dbo.CrusherWork where ks_id = '" + job_assignment_seller_id.Text + "'";
                    SqlCommand cmd = new SqlCommand(conString, con);
                ...
                if ((paymentPrice <= actualPrice) && (separatedQuantity <= actualQuantity)) {
                    try {
                        String conString = "insert into db_plastic_management.dbo.CrusherWork(e_id, ks_id, separatedQuantity, pay, advance) values('" + job_assignment_employee_id.Text + "' , '" + job_assignment_seller_id.Text + "' , '" + txt_separated_quantity.Text + "' , '" + txt_calculated_pay.Text + "' , '" + txt_advance_money.Text + "')";
                        SqlCommand cmd = new SqlCommand(conString, con);
                    ...
                    try {
                        String conString = "select loan from db_plastic_management.dbo.BottleCrusherEmployees where e_id = '" + job_assignment_employee_id.Text + "'";
                        SqlCommand cmd = new SqlCommand(conString, con);
                    ...
                    try {
                        String conString = "update db_plastic_management.dbo.BottleCrusherEmployees set loan = '" + loan + "' where e_id = '" + job_assignment_employee_id.Text + "'";
                        SqlCommand cmd = new SqlCommand(conString, con);
                    ...
                    MessageBox.Show("Unable to assign job as the Employee Crusher Quantity: \"" + separatedQuantity + "\" has exceeded the amount of the Seller Total Quantity: \"" + actualQuantity + "\".\nTherefore you can not assign further jobs on this seller.", "Amount Limit Reached", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                ...
                if (job_assignment_employee_id.Equals("")) {
                    ...
                    MessageBox.Show("You must enter the crushed quantity by an employee to finish the job assignment!", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
        }
        ...
        private void employee_update_job_Click(object sender, EventArgs e)
        {
              if(!(this.ID == -1)) {
                try {
                    String conString = "update db_plastic_management.dbo.CrusherWork set e_id = '" + job_assignment_employee_id.Text + "', ks_id = '" + job_assignment_seller_id.Text + "', separatedQuantity = '" + txt_separated_quantity.Text + "', pay = '" + txt_calculated_pay.Text + "', advance = '" + txt_advance_money.Text + "' where j_id = '" + this.ID + "'";
                    SqlCommand cmd = new SqlCommand(conString, con);
        ...
        private void employee_delete_job_Click(object sender, EventArgs e)
        {
            if (!(this.ID == -1)) {
                try {
                    String conString = "delete from db_plastic_management.dbo.CrusherWork where j_id = '" + this.ID + "'";
                    SqlCommand cmd = new SqlCommand(conString, con);
            ...
            if (!(txt_separated_quantity.Text.Equals("")) && !(job_assignment_employee_id.Text.Equals("")) && !(job_assignment_seller_id.Text.Equals(""))) {
                try {
                    ...
                    try {
                        ...
                        timer1.Stop();
                        MessageBox.Show("Couldn't convert the entered crushedQuantity as it contains some other charater than floating point numbers...", "Conversion Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        txt_separated_quantity.Text = "";
                        txt_calculated_pay.Text = "";
                        return;
                    }
                    ...
                    if (separatedQuantity > wholeQuantity) {
                        MessageBox.Show("crushedQuantity can't be greater than \'" + wholeQuantity + "\' for this seller.", "Quantity Overloaded", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        txt_separated_quantity.Text = "";
                        txt_calculated_pay.Text = "";
                        timer1.Stop();
                    } else {
        ...
        private void CrusherEmployeeManagement_Load(object sender, EventArgs e)
        {
        ...
        private void AutoFill() {
            try {
                ...
                String conString = "select e_id from db_plastic_management.dbo.BottleCrusherEmployees";
                SqlCommand cmd = new SqlCommand(conString, con);
        ...
        private void CrusherEmployeeManagement_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                ReleaseCapture();
